...
class BooksApp extends React.Component {
  ...
  bookActionsHandler = (action, selectedBook) => {
    console.log(action, selectedBook)
    selectedBook.currentShelf = action;
    let books = [...this.state.books];
    if (action === 'none') {
      this.setState({ books: books.filter(book => book.title !== selectedBook.title) })
    } else {
      books.map(book => book === selectedBook ? book.currentShelf = action : book)
  ...
  render() {
    return (
      <div className="app">
        <Route exact path="/" render={() => <MyReads books={this.state.books} handleBookActions={this.bookActionsHandler} />} />
